 ...
 * [4Android]
 */
public class ClueFind4Android
{
    private static int entryIdCounter;
    private ArrayList<String> ngramFiles;
    ...
    static {
        ClueFind4Android.entryIdCounter = 0;
    }

    public ClueFind4Android(final Config4Android c) {
        (this.ngramFiles = new ArrayList<String>()).add(c.getSubjLexicon());
        this.ngramFiles.add(c.getPolarityLexicon());
        this.ngramFiles.add(c.getIntensifierLexicon());
        this.ngramFiles.add(c.getValenceshifterLexicon());
        ...
        for (final String ngramFile : this.ngramFiles) {
            int tempIdCounter = 0;
            this.ngramList.add(new ArrayList<Entry>());
            this.wordLemmaTable.add(new Hashtable<String, ArrayList<Integer>>());
            final ArrayList<String> ngramLines = ReaderUtils4Android.readFileToLines(ngramFile);
            for (final String ngramLine : ngramLines) {
                if (ngramLine.length() != 0 && ngramLine.charAt(0) != '#') {
                    ...
                    ++tempIdCounter;
                    ++ClueFind4Android.entryIdCounter;
                }
            }
        }
    }
	...
	public Map<String, List<Annotation>> process(final List<Annotation> gateDefaultAnnotations) {
        ...
        for (int curFeatSet = 0; curFeatSet < this.ngramFiles.size(); ++curFeatSet) {
    		...
    		contentClueAnnotationFiles.put(this.ngramFiles.get(curFeatSet).split("\\.")[0], temp);
            //TODO remover caminho, deixando somente o nome do arquivo sem a extensao
    	}
